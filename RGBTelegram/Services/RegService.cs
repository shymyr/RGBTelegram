...
namespace RGBTelegram.Services
{
    public class RegService : IRegService
    {
        ...
        public async Task<PepsiRegistration> PepsiGetOrCreate(long ChatId, string phone = null)
        {
            var reg = await _context.RegistrationsPepsi.FirstOrDefaultAsync(x => x.ChatId == ChatId);

            if (reg != null) return reg;

            var result = await _context.RegistrationsPepsi.AddAsync(new PepsiRegistration() { ChatId = ChatId, phone = phone });
            await _context.SaveChangesAsync();
        ...
        public async Task<PepsiRegistration> PepsiUpdate(PepsiRegistration registration, long ChatId, string phone = null, string password = null, string first_name = null,
           string last_name = null, string middlename = null, string gender = null, string family_stat = null, string birth_day = null,
           string email = null, int? city_id = null, int? region_id = null, string iin = null)
        {
            if (!string.IsNullOrEmpty(phone))
            ...
            _context.RegistrationsPepsi.Update(registration);
            await _context.SaveChangesAsync();
